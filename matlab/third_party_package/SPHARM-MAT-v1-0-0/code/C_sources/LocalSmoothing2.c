...
void create_nbs(MAPPING *map)
{
	...
	if ((elists=(EDGELIST**)mxCalloc(vnum, sizeof(EDGELIST*)))==NULL){
...
void move_elists_to_mapnbs(MAPPING *map, EDGELIST **elists)
{
	...
	if ((map->nbs=(int**)mxCalloc(vnum, sizeof(int*)))==NULL){
...
void delete_mapping(MAPPING *map)
{
	...
	mxFree(map->faces);
	mxFree(map->obj_area);
	mxFree(map->obj_vtarea);
	mxFree(map->prm_area);
	mxFree(map->prm_vtarea);
	delete_mapnbs(map);
	mxFree(map->P);
	mxFree(map->Q);
	mxFree(map->fcix);
...
double *new_double(int size)
{
	...
	if ((newdb=(double*)mxCalloc(size, sizeof(double)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
	return newdb;
...
LEQS *new_leqs(void)
{
	...
	if ((leqs=(LEQS*)mxCalloc(1,sizeof(LEQS)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
...
MAPPING *new_mapping(void)
{
	...
	if ((map=(MAPPING*)mxCalloc(1,sizeof(MAPPING)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
	map->param_vs = NULL;
...
{
	int *newint;
	if ((newint=(int*)mxCalloc(size,sizeof(int)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
	return newint;
...
{
	EDGELIST *elist;
	if ((elist=(EDGELIST*)mxCalloc(1,sizeof(EDGELIST)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
...
REGMESH *new_regmesh(int n)
{
	...
	if ((regmesh=(REGMESH*)mxCalloc(1,sizeof(REGMESH)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
...
double **new_dbarray(int n)
{
	...
	if ((A=(double**)mxCalloc(n, sizeof(double*)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
	if ((data=(double*)mxCalloc(n*n,sizeof(double)))==NULL){
		fprintf(stderr, "not enough memory\n");
		exit(1);
	}
